...
export function CncOperatorWorkspace({ machine, job, onComplete, onRecordProduction, onOffcut, onScrap }: WorkspaceProps) {
  const [boardArea, setBoardArea] = useState(String(job?.quantity ?? ""));
  ...
  return (
    ...
    >
      ...
      <div className="operator-form-grid">
        <label>Board area (m²)<NumericInput min={0} step="0.1" value={boardArea} onChange={setBoardArea} /></label>
      ...
      <div className="control-readout"><Command size={17} /><span>Calculated routing passes</span><b>{Math.max(1, Math.ceil(Number(boardArea) / 2.5))}</b></div>
    </OperatorWorkspaceShell>
  );
}
