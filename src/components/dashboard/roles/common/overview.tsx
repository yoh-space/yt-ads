...
function trendLabel(current: number, previous: number): string {
  if (previous <= 0)
    return current > 0 ? "new vs yesterday" : "no change vs yesterday";
...
export function Overview({
  ...
  const todaysOrderCount =
    financialMetrics?.todaysOrderCount ?? orderStats.todaysOrders;
  ...
  const inPrintCount = jobs.filter(
  ...
  const queueStages = [
    ...
    { label: "In Print", value: inPrintCount, active: inPrintCount > 0 },
    {
      ...
      active: inPrintCount === 0 && completedTodayCount > 0,
    },
  ];
  const quotaDenominator = queuedCount + inPrintCount + completedTodayCount;
  const quotaPace =
    quotaDenominator > 0 ? (completedTodayCount / quotaDenominator) * 100 : 0;
  const fleetUptime = machines.length
    ? machines.reduce(
        (total, machine) => total + machineUptime(machine.status),
        0
      ) / machines.length
    : 0;
  ...
  return (
    <>
      ...
      <section
        className="grid grid-cols-3 gap-3 p-3.5 mb-[18px] rounded-xl bg-card border border-border/60"
        aria-label="ዋና መለያዎች"
      >
        <StatCard
          variant="sales"
        ...
        <StatCard
          ...
          value={
            profitNegative
              ? `-${formatEtb(Math.abs(todaysNetProfit))}`
              : formatEtb(todaysNetProfit)
          }
        ...
        <StatCard
          ...
          metadata={
            kpis
              ? trendLabel(kpis.todaysOrdersCount, kpis.yesterdayOrdersCount)
              : undefined
          }
        />
      ...
      {kpis ? (
        <section
          className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-[18px]"
        ...
        >
          ...
          <StatCard
            ...
            metadata={trendLabel(
              kpis.todaysCompletedCount,
              kpis.yesterdayCompletedCount
            )}
          />
      ...
      <section
        className="grid grid-cols-1 md:grid-cols-3 gap-[14px] mb-[14px]"
        aria-label="የማሽኖች ወቅታዊ መጠን"
      >
        <MetricChart
        ...
        <Panel>
          ...
          <div className="px-[17px] py-[7px]">
            {materials.slice(0, 4).map(material => {
              const pulse = materialPulse.find(
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <span className="block mt-[3px] text-muted-foreground text-[11px]">
                      ...
                      · {formatQuantity(consumed, material.unit)} used 30d
                    </span>
                  </div>
            ...
            {jobs
              ...
              .map(job => {
                const machine = machines.find(
                  item => item.id === job.machineId
                );
                const material = materials.find(
                  item => item.id === job.materialId
                );
                return (
                  <div
                    key={job.id}
                  ...
                  >
                    <div className="flex flex-col gap-[3px]">
                      <b className="text-foreground font-mono font-bold text-[10px] tracking-[0.08em]">
                        {job.code}
                      </b>
                    ...
                    <StatusPill variant={statusTone(job.status)}>
                      {job.status}
                    </StatusPill>
                    <button
                      ...
                      onClick={() =>
                        job.status === "In production" && onComplete(job.id)
                      }
      ...
      <section
        className={cn(
          "grid gap-[14px]",
          role === "owner" ? "grid-cols-1" : "grid-cols-[1.55fr_1fr]"
        )}
      >
        {role === "owner" ? null : (
          <Panel>
            ...
            <div className="grid grid-cols-5 gap-2 px-[17px] pt-1 pb-[10px]">
              <div className="flex flex-col items-center p-[9px_4px] border border-[#1e293b] rounded-[7px] bg-[#0b1220] text-center">
                <strong className="text-foreground font-bold text-lg leading-none tracking-[-0.5px] font-mono tabular-nums">
                  {machines.length}
                </strong>
                <span className="mt-[3px] text-muted-foreground text-[9px] font-semibold uppercase tracking-[0.3px] font-mono">
                  Total
                </span>
              </div>
            ...
            <div className="px-[17px] pb-2">
              {machines.map(machine => (
                <div
                  key={machine.id}
                  className="grid grid-cols-[30px_minmax(0,1fr)_auto_auto] gap-[9px] items-center py-2 border-b border-[#16202f] last:border-b-0"
                >
                  <div
                    className={cn(
                      "grid place-items-center w-7 h-7 rounded-md bg-[#0b1220] text-muted-foreground border border-[#1e293b]",
                      {
                        "text-cyan bg-cyan/15 border-cyan/30":
                  ...
                  <div className="flex flex-col gap-[1px] min-w-0">
                    <div className="flex items-baseline gap-[6px]">
                      ...
                      <span className="text-muted-foreground font-mono text-[9px] tracking-[0.08em]">
                        {machine.code}
                      </span>
                    </div>
                  ...
                  <StatusPill variant={statusTone(machine.status)}>
                    {machine.status}
                  </StatusPill>
