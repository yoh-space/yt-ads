          ...
          {!detail ? <LoadingBody /> : <div className="space-y-5">
            ...
            <section>
              ...
              <div className="rounded-xl border border-border/60 bg-background/30">
                <div className="border-b border-border/50 px-4 py-3"><p className="text-sm font-semibold text-foreground">{detail.serviceType}</p><p className="mt-1 text-[11px] text-muted-foreground">{detail.dimensions} · Quantity {detail.quantity}</p>{detail.specifications && Object.keys(detail.specifications).length > 0 ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{Object.entries(detail.specifications).map(([key, value]) => <DetailRow key={key} label={key.replaceAll(/([A-Z])/g, " $1")} value={value} />)}</div> : null}</div>
