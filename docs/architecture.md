...
| Validation | **Convex validators (`convex/values`) + `convex/validation.ts`** | Shared typed validator objects reused by every public mutation/query. |
...
| Hand a batch to a press operator | `STORE_TO_OPERATOR_TRANSFER` | `parent` (when issued by storekeeper) | `transfer` | A storekeeper "issues" to an operator (`convex/materialRequests.ts:122` → `recordInventoryEvent`). | Catalog base-units ↓, operator sub-stock `currentRemaining` ↑, issued quantity `+baseQuantity`, packaging units `+packageQuantity`. |
...
The single helper that writes every ledger row enforces these invariants transactionally (`convex/inventoryLedger.ts:51`):

1. **Quantities are strictly positive.** `baseQuantity > 0` is asserted (`inventoryLedger.ts:55`).
2. **Notes are never empty** — every event has a free-text narrative (`inventoryLedger.ts:58`).
3. **The material must be active** (`inventoryLedger.ts:60`).
4. **Central-store balance cannot go negative**; the projection rejects with `Insufficient ${material.name} stock` (`inventoryLedger.ts:71`). The projection clamps to zero on read (`materials.quantity = Math.max(0, …)`) but rejects on write.
5. **Operator sub-stock cannot go negative** (`inventoryLedger.ts:111`). A sub-stock row also auto-flips to `EXHAUSTED` when remaining ≤ 0 (`inventoryLedger.ts:117`). Then a separate `exhaustOperatorStock` (and the weekly reconciliation flow) is responsible for transitioning it through `PENDING_CLEARANCE → CLEARED`.
6. **The event is appended *after* the projection updates** (`inventoryLedger.ts:122`), so readers replaying the ledger see the same state the projections show.
...
| `machines` | Production resources. Each row carries `operatorRole` so ABAC can answer "is this operator allowed to drive this machine?" — see `convex/authorization.ts:201`. | `by_code`, `by_operator_role` |
...
| Table | Purpose | Key indices |
| --- | --- | --- |
| `materialRequests` | Per-job-card request to the store for a given material. Drives the "Issue / Acknowledge" UI inside the dashboard (`src/components/dashboard/material-requests-panel.tsx`). | `by_job_card`, `by_status` |
| `stockMovements` | Simplified legacy summary stream used by the audit view. The authoritative stream is `stock_movements`; this table is kept for audit-log display only. | `by_material` |
...
| `parentInventory` | Whole-unit central projection, keyed by material with `unitType ∈ {ROLL, SHEET, LITER}` and unit-specific conversion factor. | `by_material`, `by_unit_type` |
| `operatorMachineStock` | Legacy floor projection. Kept to bridge old `weeklyReconciliations.operatorStockId`; new work targets `operatorSubStock`. | `by_item`, `by_machine`, `by_material_machine`, `by_status` |
| `operatorSubStock` | Active floor projection. Carries `parentInventoryId`, `operatorId`, `machineId`, the issuance counters (`issuedUnits`, `issuedQuantity`, `currentRemaining`), `status ∈ {ACTIVE, PENDING_CLEARANCE, CLEARED, EXHAUSTED}`, and the owner clearance trail (`clearedBy`, `clearedAt`, `clearanceNote`). | `by_parent_inventory`, `by_machine`, `by_operator`, `by_material_machine`, `by_status` |
